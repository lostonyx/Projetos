...
class ModelPaymentPPExpress extends Model {
    ...
    public function call($data) {
        ...
        $settings = array(
            ...
            'SIGNATURE' => $signature,
            'VERSION' => '109.0',
            'BUTTONSOURCE' => 'iwnetworkstore_2.0_EC',
        );
    ...
    public function paymentRequestInfo() {
        ...
        foreach ($this->cart->getProducts() as $item) {
            ...
            $item_price = $this->currency->format($item['price'], $this->currency->getCode(), 1, false);
        ...
        foreach ($total_data as $total_row) {
            if (!in_array($total_row['code'], array('total', 'sub_total'))) {
                if ($total_row['value'] != 0) {
                    $item_price = $this->currency->format($total_row['value'], $this->currency->getCode(), 1, false);

                    $data['L_PAYMENTREQUEST_0_NUMBER' . $i] = $total_row['code'];
                    $data['L_PAYMENTREQUEST_0_NAME' . $i] = $total_row['title'];
                    $data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($total_row['value'], $this->currency->getCode(), 1, false);
