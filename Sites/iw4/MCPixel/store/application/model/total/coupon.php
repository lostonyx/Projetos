...
class ModelTotalCoupon extends Model {
    public function getCoupon($code) {
        ...
        $coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '2'");

        if ($coupon_query->num_rows) {
            if ($this->config->get("coupon_top10_status")) {
                if (in_array($coupon_query['code'], $this->config->get("coupon_top10_coupons"))) {
