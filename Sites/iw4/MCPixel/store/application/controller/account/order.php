...
class ControllerAccountOrder extends Controller {
    ...
    public function info() {
        ...
        if ($order_info) {
            ...
            $data['text_playername'] = $this->language->get('text_playername');
            ...
            // Player
            $cf = $order_info['custom_field'];
            $player = chk_array($cf, $this->config->get("config_custom_field_player"));
            
            $data['playername'] = '';
            if ($player) {
                $pl = new Player($this->registry);
                $query = $pl->getPlayerByUUID($player);
                if (!isset($query['error'])) {
                    $data['playername'] = $query['name'];
                }
            }
        ...
        if ($order_info) {
            ...
            if ($order_product_info) {
                ...
                if ($product_info) {
                    ...
                    $this->cart->add($order_product_info['product_id'], $order_product_info['quantity']);

                    $this->session->data['success'] = sprintf($this->language->get('text_success'), $product_info['name']);
