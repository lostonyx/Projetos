...
class ModelCatalogRecurring extends Model {
	public function addRecurring($data) {
		...
		foreach ($data['recurring_lang'] as $language_id => $recurring_lang) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "recurring_lang` (`recurring_id`, `language_id`, `name`) VALUES (" . (int)$recurring_id . ", " . (int)$language_id . ", '" . $this->db->escape($recurring_lang['name']) . "')");
		}
	...
	public function editRecurring($recurring_id, $data) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "recurring_lang` WHERE recurring_id = '" . (int)$recurring_id . "'");
	...
	public function copyRecurring($recurring_id) {
		...
		$data['recurring_lang'] = $this->getRecurringDescription($recurring_id);

		foreach ($data['recurring_lang'] as &$recurring_lang) {
			$recurring_lang['name'] .= ' - 2';
		}
	...
	public function getRecurringDescription($recurring_id) {
		$recurring_lang_data = array();

		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "recurring_lang` WHERE `recurring_id` = '" . (int)$recurring_id . "'");

		foreach ($query->rows as $result) {
			$recurring_lang_data[$result['language_id']] = array('name' => $result['name']);
		}

		return $recurring_lang_data;
	}

	public function getRecurrings($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "recurring` r LEFT JOIN " . DB_PREFIX . "recurring_lang rd ON (r.recurring_id = rd.recurring_id) WHERE rd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
