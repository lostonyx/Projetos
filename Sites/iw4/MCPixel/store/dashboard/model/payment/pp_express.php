...
class ModelPaymentPPExpress extends Model {
	...
	public function updateOrder($capture_status, $order_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "paypal_order` SET `date_updated` = now(), `capture_status` = '" . $this->db->escape($capture_status) . "' WHERE `order_id` = '" . (int)$order_id . "'");
	}
	...
	public function call($data) {
		...
		$settings = array(
			...
			'SIGNATURE' => $signature,
			'VERSION' => '84',
			'BUTTONSOURCE' => 'IWNetwork Store_Cart_EC',
		);
