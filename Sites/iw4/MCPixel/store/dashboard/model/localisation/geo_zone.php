...
class ModelLocalisationGeoZone extends Model {
	...
	public function editGeoZone($geo_zone_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "geo_zone SET name = '" . $this->db->escape($data['name']) . "', description = '" . $this->db->escape($data['description']) . "', date_updated = NOW() WHERE geo_zone_id = '" . (int)$geo_zone_id . "'");
