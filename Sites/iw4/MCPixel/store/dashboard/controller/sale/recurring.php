...
class ControllerSaleRecurring extends Controller {
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . $url, 'SSL'),
		);
		...
		foreach ($results as $result) {
			...
			$data['recurrings'][] = array(
				...
				'order_link'         => $this->url->link('dashboard/sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL'),
				'reference'          => $result['reference'],
				'customer'           => $result['customer'],
				'status'             => $result['status'],
				'date_added'         => $date_added,
				'view'               => $this->url->link('dashboard/sale/recurring/info', 'token=' . $this->session->data['token'] . '&order_recurring_id=' . $result['order_recurring_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_order_recurring'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=or.order_recurring_id' . $url, 'SSL');
		$data['sort_order'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=or.order_id' . $url, 'SSL');
		$data['sort_reference'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=or.reference' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=or.status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&sort=or.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . '&page={page}' . $url, 'SSL');
	...
	public function info() {
		...
		if ($order_recurring) {
			...
			$data['entry_recurring_lang'] = $this->language->get('entry_recurring_lang');
			...
			$data['recurring_lang'] = $order_recurring['recurring_lang'];
			$data['recurring_name'] = $order_recurring['recurring_name'];

			$data['order_id'] = $order['order_id'];
			$data['order_href'] = $this->url->link('dashboard/sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order['order_id'], 'SSL');
			...
			if ($order['customer_id']) {
				$data['customer_href'] = $this->url->link('dashboard/customer/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $order['customer_id'], 'SSL');
			} else {
				$data['customer_href'] = '';
			}

			if ($order_recurring['recurring_id'] != '0') {
				$data['recurring'] = $this->url->link('dashboard/catalog/recurring/edit', 'token=' . $this->session->data['token'] . '&recurring_id=' . $order_recurring['recurring_id'], 'SSL');
			} else {
				$data['recurring'] = '';
			}
			...
			$data['return'] = $this->url->link('dashboard/sale/recurring', 'token=' . $this->session->data['token'] . $url, 'SSL');
