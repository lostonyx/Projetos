...
class ControllerCatalogRecurring extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('dashboard/catalog/recurring/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['copy'] = $this->url->link('dashboard/catalog/recurring/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('dashboard/catalog/recurring/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['recurrings'][] = array(
				...
				'edit'         => $this->url->link('dashboard/catalog/recurring/edit', 'token=' . $this->session->data['token'] . '&recurring_id=' . $result['recurring_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['recurring_id'])) {
			$data['action'] = $this->url->link('dashboard/catalog/recurring/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('dashboard/catalog/recurring/edit', 'token=' . $this->session->data['token'] . '&recurring_id=' . $this->request->get['recurring_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('dashboard/catalog/recurring', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['recurring_lang'])) {
			$data['recurring_lang'] = $this->request->post['recurring_lang'];
		} elseif (!empty($recurring_info)) {
			$data['recurring_lang'] = $this->model_catalog_recurring->getRecurringDescription($recurring_info['recurring_id']);
		} else {
			$data['recurring_lang'] = array();
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['recurring_lang'] as $language_id => $value) {
