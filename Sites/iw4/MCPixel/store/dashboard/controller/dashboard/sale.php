...
class ControllerDashboardSale extends Controller {
	public function index() {
		if (!$this->user->hasPermission('access', 'dashboard/sale')) {
			return false;
		}
		...
		$sale_total = $this->model_report_sale->getTotalSales(array("status" => $this->config->get("config_complete_status")));
		...
		$data['sale'] = $this->url->link('dashboard/sale/order', 'token=' . $this->session->data['token'], 'SSL');
