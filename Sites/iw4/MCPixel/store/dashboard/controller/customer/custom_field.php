...
class ControllerCustomerCustomField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('dashboard/customer/custom_field/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('dashboard/customer/custom_field/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			switch ($result['type']) {
				...
				case 'text':
					...
					break;
				case 'player2uuid':
					$type = $this->language->get('text_player2uuid');
					break;
				case 'textarea':
			...
			$data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link('dashboard/customer/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . '&sort=cfd.name' . $url, 'SSL');
		$data['sort_location'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.location' . $url, 'SSL');
		$data['sort_type'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.type' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.status' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('customer/custom_field_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		$data['text_player2uuid'] = $this->language->get('text_player2uuid');
		...
		if (!isset($this->request->get['custom_field_id'])) {
			$data['action'] = $this->url->link('dashboard/customer/custom_field/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('dashboard/customer/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('dashboard/customer/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['custom_field_lang'])) {
			$data['custom_field_lang'] = $this->request->post['custom_field_lang'];
		} elseif (isset($this->request->get['custom_field_id'])) {
			$data['custom_field_lang'] = $this->model_customer_custom_field->getCustomFieldDescriptions($this->request->get['custom_field_id']);
		} else {
			$data['custom_field_lang'] = array();
		}
		...
		foreach ($custom_field_values as $custom_field_value) {
			$data['custom_field_values'][] = array(
				...
				'custom_field_value_lang' => $custom_field_value['custom_field_value_lang'],
		...
		$this->response->setOutput($this->load->view('customer/custom_field_form.tpl', $data));
	}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['custom_field_lang'] as $language_id => $value) {
		...
		if (($this->request->post['type'] == 'select' || $this->request->post['type'] == 'radio' || $this->request->post['type'] == 'checkbox')) {
			...
			if (isset($this->request->post['custom_field_value'])) {
				foreach ($this->request->post['custom_field_value'] as $custom_field_value_id => $custom_field_value) {
					foreach ($custom_field_value['custom_field_value_lang'] as $language_id => $custom_field_value_lang) {
						if ((utf8_strlen($custom_field_value_lang['name']) < 1) || (utf8_strlen($custom_field_value_lang['name']) > 128)) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $custom_field_id) {
			if ($custom_field_id == '1') {
				$this->error['warning'] = sprintf($this->language->get("error_player2uuid"), $this->language->get("text_player2uuid"));
			}
		}
