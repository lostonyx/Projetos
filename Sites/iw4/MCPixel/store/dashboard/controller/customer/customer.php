...
class ControllerCustomerCustomer extends Controller {
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );

        $data['add'] = $this->url->link('dashboard/customer/customer/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['delete'] = $this->url->link('dashboard/customer/customer/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($results as $result) {
            if (!$result['approved']) {
                $approve = $this->url->link('dashboard/customer/customer/approve', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL');
            } else {
                $approve = '';
            }
            ...
            if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
                $unlock = $this->url->link('dashboard/customer/customer/unlock', 'token=' . $this->session->data['token'] . '&email=' . $result['email'] . $url, 'SSL');
            } else {
                $unlock = '';
            }
            ...
            $data['customers'][] = array(
                ...
                'approve' => $approve,
                'unlock' => $unlock,
                'edit' => $this->url->link('dashboard/customer/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL')
            );
        }
        ...
        $data['sort_name'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
        $data['sort_email'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, 'SSL');
        $data['sort_customer_group'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=customer_group' . $url, 'SSL');
        $data['sort_status'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
        $data['sort_ip'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=c.ip' . $url, 'SSL');
        $data['sort_date_added'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
    ...
    protected function getForm() {
        ...
        if (!isset($this->request->get['customer_id'])) {
            $data['action'] = $this->url->link('dashboard/customer/customer/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $data['action'] = $this->url->link('dashboard/customer/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
        }

        $data['cancel'] = $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($custom_fields as $custom_field) {
            $data['custom_fields'][$custom_field['custom_field_id']] = array(
        ...
        // Custom Fields > Playername
        $player = new Player($this->registry);

        foreach ($data['custom_fields'] as $cf_id => $cf) {
            if ($cf['type'] == "player2uuid") {
                $value = (isset($data['account_custom_field'][$cf_id]) ? $data['account_custom_field'][$cf_id] : false);
                if (!$value) {
                    continue;
                }
                $player = $player->getPlayerByUUID($value);
                if (empty($player) || isset($player['error'])) {
                    continue;
                }
                $data['account_custom_field'][$cf_id] = $player['name'];
            }
        }
    ...
    protected function validateForm() {
        ...
        foreach ($custom_fields as $custom_field) {
            if (($custom_field['location'] == 'account') && $custom_field['required'] && empty($this->request->post['custom_field'][$custom_field['custom_field_id']])) {
                ...
                if ($custom_field['type'] == "player2uuid") {
                    $value = str_replace("-", "", $this->request->post['custom_field'][$custom_field['custom_field_id']]);
                    $pl = (is_md5($value)) ? $player->getPlayerByUUID($value) : $player->getPlayerByName($value);
                    if (empty($pl) || isset($pl['error'])) {
                        $this->error['custom_field'][$custom_field['custom_field_id']] = $this->language->get('error_cf_player_invalid');
                    } else {
                        $this->request->post['custom_field'][$custom_field['custom_field_id']] = $pl['id'];
                    }
                }
            }
        }
    ...
    public function login() {
        ...
        if ($customer_info) {
            ...
            $this->response->redirect($this->config->get("config_url") . 'account/login?token=' . $token);
        } else {
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('dashboard/error/not_found', 'token=' . $this->session->data['token'], 'SSL')
            );
    ...
    public function history() {
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('dashboard/customer/customer/history', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
    ...
    public function transaction() {
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('dashboard/customer/customer/transaction', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
    ...
    public function ip() {
        ...
        foreach ($results as $result) {
            $data['ips'][] = array(
                ...
                'filter_ip' => $this->url->link('dashboard/customer/customer', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], 'SSL')
            );
        }
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('dashboard/customer/customer/ip', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
