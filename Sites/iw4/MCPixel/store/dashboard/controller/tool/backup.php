	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'tool/backup')) {
			...
			if ($content) {
				...
				$this->response->redirect($this->url->link('dashboard/tool/backup', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('dashboard/tool/backup', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['restore'] = $this->url->link('dashboard/tool/backup', 'token=' . $this->session->data['token'], 'SSL');

		$data['backup'] = $this->url->link('dashboard/tool/backup/backup', 'token=' . $this->session->data['token'], 'SSL');
