...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('dashboard/localisation/geo_zone/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('dashboard/localisation/geo_zone/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['geo_zones'][] = array(
				...
				'edit'        => $this->url->link('dashboard/localisation/geo_zone/edit', 'token=' . $this->session->data['token'] . '&geo_zone_id=' . $result['geo_zone_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_lang'] = $this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . '&sort=description' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->error['description'])) {
			$data['error_lang'] = $this->error['description'];
		} else {
			$data['error_lang'] = '';
		}
		...
		if (!isset($this->request->get['geo_zone_id'])) {
			$data['action'] = $this->url->link('dashboard/localisation/geo_zone/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('dashboard/localisation/geo_zone/edit', 'token=' . $this->session->data['token'] . '&geo_zone_id=' . $this->request->get['geo_zone_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('dashboard/localisation/geo_zone', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['description']) < 3) || (utf8_strlen($this->request->post['description']) > 255)) {
			$this->error['description'] = $this->language->get('error_lang');
		}
