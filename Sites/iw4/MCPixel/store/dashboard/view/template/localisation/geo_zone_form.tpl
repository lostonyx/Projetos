          ...
          <table id="zone-to-geo-zone" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr id="zone-to-geo-zone-row<?php echo $zone_to_geo_zone_row; ?>">
                <td class="text-left"><select name="zone_to_geo_zone[<?php echo $zone_to_geo_zone_row; ?>][country_id]" id="country<?php echo $zone_to_geo_zone_row; ?>" class="form-control" onchange="$('#zone<?php echo $zone_to_geo_zone_row; ?>').load('dashboard/localisation/geo_zone/zone?token=<?php echo $token; ?>&country_id=' + this.value + '&zone_id=0');">
...
$('#zone-id').load('dashboard/localisation/geo_zone/zone?token=<?php echo $token; ?>&country_id=' + $('#country-id').attr('value') + '&zone_id=0');
//--></script>
  <?php $zone_to_geo_zone_row = 0; ?>
  <?php foreach ($zone_to_geo_zones as $zone_to_geo_zone) { ?>
  <script type="text/javascript"><!--
$('#zone<?php echo $zone_to_geo_zone_row; ?>').load('dashboard/localisation/geo_zone/zone?token=<?php echo $token; ?>&country_id=<?php echo $zone_to_geo_zone['country_id']; ?>&zone_id=<?php echo $zone_to_geo_zone['zone_id']; ?>');
//--></script>
  <?php $zone_to_geo_zone_row++; ?>
  <?php } ?>
  <script type="text/javascript"><!--
...
function addGeoZone() {
	...
	html += '  <td class="text-left"><select name="zone_to_geo_zone[' + zone_to_geo_zone_row + '][country_id]" id="country' + zone_to_geo_zone_row + '" class="form-control" onchange="$(\'#zone' + zone_to_geo_zone_row + '\').load(\'dashboard/localisation/geo_zone/zone?token=<?php echo $token; ?>&country_id=\' + this.value + \'&zone_id=0\');">';
	<?php foreach ($countries as $country) { ?>
	...
	$('#zone' + zone_to_geo_zone_row).load('dashboard/localisation/geo_zone/zone?token=<?php echo $token; ?>&country_id=' + $('#country' + zone_to_geo_zone_row).attr('value') + '&zone_id=0');
