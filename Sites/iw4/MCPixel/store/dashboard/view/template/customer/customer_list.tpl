                  ...
                  <td class="text-right"><?php if ($customer['approve']) { ?>
                    ...
                    <div class="btn-group" data-toggle="tooltip" title="<?php echo $button_login; ?>">
                      ...
                      <ul class="dropdown-menu pull-right">
                        <li><a href="dashboard/customer/customer/login?token=<?php echo $token; ?>&customer_id=<?php echo $customer['customer_id']; ?>&store_id=0" target="_blank"><?php echo $text_default; ?></a></li>
                      </ul>
                    </div>
...
$('#button-filter').on('click', function() {
	url = 'dashboard/customer/customer?token=<?php echo $token; ?>';
...
$('input[name=\'filter_email\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'dashboard/customer/customer/autocomplete?token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request),
			dataType: 'json',			
			success: function(json) {
				response($.map(json, function(item) {
					return {
