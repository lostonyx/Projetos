...
public class RexAPI {
	...
	public static void sendPacket(Object packet, Player player) throws Exception {
		...
		getResult(getConnection(player), "sendPacket", getParameters(pPacket), packet);
	}
	...
	public static void setTabList(Player player, String header, String footer) {
		try {
			if (isAbove1_8(player)) {
				Object packet = getNew(sPacketTabHeader, getParameters(mIChatBaseComponent, mIChatBaseComponent),
		...
		try {
			Object packet = getNew(pPlayOutPlayerListHeaderFooter, getParameters(mIChatBaseComponent),
					getIChatText(header));
	...
	public static boolean isAbove1_8(Player player) {
		try {
			return (int) getResult(getValue(getConnection(player), "networkManager"), "getVersion") == 47;
	...
	public static void sendTitle(Player player, String title, String subTitle, int fadeIn, int stay, int fadeOut) {
		try {
			if (isAbove1_8(player)) {
				...
				sendPacket(player, getNew(sPacketTitle, getValue(sAction, "TIMES"), fadeIn, stay, fadeOut));
				sendPacket(player, getNew(sPacketTitle, getParameters(sAction, mIChatBaseComponent),
		...
		try {
			...
			sendPacket(player, getNew(pPlayOutTitle, getParameters(cEnumTitleAction, mIChatBaseComponent),
					getValue(cEnumTitleAction, "TITLE"), getIChatText(title)));
			sendPacket(player, getNew(pPlayOutTitle, getParameters(cEnumTitleAction, mIChatBaseComponent),
					getValue(cEnumTitleAction, "SUBTITLE"), getIChatText(subTitle)));
			return;
		} catch (Exception e) {
		}
		try {
			sendPacket(player, getNew(pPlayOutTitle, fadeIn, stay, fadeOut));
			sendPacket(player, getNew(pPlayOutTitle, getParameters(pEnumTitleAction2, mIChatBaseComponent),
					getValue(pEnumTitleAction2, "TITLE"), getIChatText2(title)));
			sendPacket(player, getNew(pPlayOutTitle, getParameters(pEnumTitleAction2, mIChatBaseComponent),
					getValue(pEnumTitleAction2, "SUBTITLE"), getIChatText2(subTitle)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static void sendActionBar(Player player, String text) {
		try {
			...
			Object packet = getNew(pPlayOutChat, getParameters(mIChatBaseComponent, byte.class), component, (byte) 2);
	...
	public static void makeRespawn(Player player) {
		try {
			Object packet = getNew(pPlayInClientCommand, getValue(mEnumClientCommand, "PERFORM_RESPAWN"));
			getResult(getConnection(player), "a", packet);

		} catch (Exception ex) {
			try {
				Object packet = getNew(pPlayInClientCommand, getValue(mEnumClientCommand2, "PERFORM_RESPAWN"));
				getResult(getConnection(player), "a", packet);
			} catch (Exception e) {
			}
	...
	public static void changeName(Player player, String displayName) {
		...
		try {
			Object packet = getNew(pPlayOutNamedEntitySpawn, getParameters(mEntityHuman), getHandle(player));
